...
* **백엔드(Backend):** Spring Boot 3.3.x, Java 17, Spring Security (HttpOnly 쿠키 기반 JWT), JPA (PostgreSQL).
...
* `core/`: 공통 설정 및 인프라 클래스 (JacksonConfig, GlobalExceptionHandler, DataInitializer 등).
...
* `src/shared/components/ui/`: shadcn/ui 컴포넌트 프리셋.
...
* **헬스 체크 (Health Check):** `http://localhost:9595/sofia/health` (또는 `/sofia/api/health`)
